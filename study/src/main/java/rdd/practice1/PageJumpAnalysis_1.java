 ...
 * 计算指定的页面的跳转转换率
 */
public class PageJumpAnalysis_1 {
    public static void main(String[] args) {
        ...
        HashSet<Long> pageIdSet = new HashSet<>(Arrays.asList(1L, 2L, 3L, 4L, 5L, 6L, 7L));
        ...
        JavaPairRDD<Long, Integer> pageCnt = actionRDD.filter((Function<ActionData, Boolean>) data->{
            return pageIdSet.contains(data.pageId);
        }).mapToPair((PairFunction<ActionData, Long, Integer>) action -> {
        ...
        List<Tuple2<Tuple2<Long, Long>, Integer>> jumpTimes = jumpTimesRDD.filter((Function<Tuple2<Tuple2<Long, Long>, Integer>, Boolean>) tuple->{
            Long pageId1 = tuple._1()._1();
            Long pageId2 = tuple._1()._2();
            return pageId1>=1L&&pageId1<=6&&pageId2==pageId1+1L;
        }).collect();
